...
cutadapt -a $IR...CCCCCCCCCC -M 145 -m 20 -q 20 --discard-untrimmed -j $SLURM_CPUS_PER_TASK -o $PREFIX.trim.fastq $PREFIX.fastq >$PREFIX.cutadapt_log.txt
...
bowtie2 --end-to-end -p $SLURM_CPUS_PER_TASK -a -x $GENOME -U $PREFIX.trim.fastq -S $PREFIX.sam 2>> $PREFIX-TnSeq.txt
